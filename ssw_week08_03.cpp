...
int main() {
    ...
    while (!readFile.eof()) {    // 해당 파일이 끝(EOF)이 아닐 경우
        ...
        arr.opcode[i].erase(arr.opcode[i].end());
    ...
    int j = 0;
    int locctr = 1000;
    int hexlocctr = locctr + 3096;
    int flag = 0;
    cout << "Label: " << arr.label[j] << ",Loc: " << locctr << "\t" << ",flag: " << flag << endl;
    j++;
    while(j<12) {
        if(arr.label[j] != "-\t\t") {
            cout << "Label: " << arr.label[j] << ",Loc: " << hex << hexlocctr << "\t" << ",flag: " << flag << endl;
        }

        hexlocctr += 3;

        for(int i=0; i<12; i++) {
            if(arr.operand[j] == arr.label[i])
                flag = 1;
        }
        j++;
    }
